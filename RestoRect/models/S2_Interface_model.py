...
class S2_Interface_Model(SRModel):
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # FIXED: Proper tensor to image conversion
            # Clamp output to valid range and handle denormalization properly
            ...
            output_img = output_tensor.detach().cpu().permute(1, 2, 0).numpy()
            
            # Convert RGB to BGR for OpenCV (since training used bgr2rgb=True)
            output_img = cv2.cvtColor(output_img, cv2.COLOR_RGB2BGR)
            
            # Convert to 8-bit
            sr_img = (output_img * 255.0).astype(np.uint8)
            ...
            if 'gt' in visuals:
                ...
                gt_img = gt_tensor.detach().cpu().permute(1, 2, 0).numpy()
                gt_img = cv2.cvtColor(gt_img, cv2.COLOR_RGB2BGR)
                gt_img = (gt_img * 255.0).astype(np.uint8)
    ...
    def test(self):
        ...
        # Use the set number of steps or default to 5
        num_steps = getattr(self, 'rf_steps', 5)
        
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                # Pass num_steps to the model for RF inference
                self.output = self.net_g_ema.test_with_steps(lq, retinex_lq, num_steps)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g.test_with_steps(lq, retinex_lq, num_steps)
            self.net_g.train()
    ...
    def set_rf_steps(self, steps):
        """Set the number of steps for Rectified Flow inference"""
        self.rf_steps = steps

    def validation_with_fid(self, dataloader, current_iter, tb_logger, save_img):
        """Validation with FID calculation using custom implementation"""
        import tempfile
        import os
        import cv2
        import numpy as np
        ...
        # Copy the FID implementation from evaluation.py
        ...
        def calculate_fid_custom(real_images, fake_images, device='cuda'):
        ...
        # Collect images in memory for FID calculation
        real_images = []
        fake_images = []
        ...
        try:
            if len(real_images) > 0 and len(fake_images) > 0:
                fid_score = calculate_fid_custom(real_images, fake_images, device='cuda')
            else:
                fid_score = float('inf')
        ...
        logger = get_root_logger()
        logger.info(f'FID for {self.rf_steps} steps: {fid_score:.2f}')
