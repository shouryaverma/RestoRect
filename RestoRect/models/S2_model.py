...
class RestoRect_S2Model(SRModel):
        ...
        if hasattr(self, 'use_flex') and self.use_flex:
            ...
            try:
                self.cri_flex = FlexLoss(
                    ...
                    outlier_percentile=flex_opt.get('outlier_percentile', 95.0),
    ...
    def optimize_parameters(self, current_iter):
        ...
        # Get S1 features - keep gradients for Phase 2
        if current_iter < self.encoder_iter:
            with torch.no_grad():
            ...
            # Allow gradients to flow in Phase 2
            _, S1_IPR_rex = self.model_Es1_rex(self.retinex_lq, self.retinex_gt)
            _, S1_IPR_img = self.model_Es1_img(self.lq, self.gt)
        ...
        if current_iter < self.encoder_iter:
            ...
            # Use training forward pass instead of inference
            ...
            if self.cri_flex is not None:
                teacher_features = [S1_IPR_rex[0], S1_IPR_img[0]]
                student_features = [pred_IPR_list[0][i_rex], pred_IPR_list[1][i_img]]
