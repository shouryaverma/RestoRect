...
class RestoRectS2_Interface(nn.Module):
    def __init__(self,
                 n_encoder_res=6,
                 inp_channels=3,
                 out_channels=3,
                 ...
                 linear_end=0.99,
                 timesteps=4):
        super(RestoRectS2_Interface, self).__init__()
    ...
    def test_with_steps(self, img, retinex, num_steps=5):
        """Test with specified number of RF steps"""
        # Set the number of steps for both diffusion models
        IPRS2_rex, _ = self.rex_diffusion.inference_generation(retinex, num_steps=num_steps)
        IPRS2_img, _ = self.img_diffusion.inference_generation(img, num_steps=num_steps)
